...
if [ "$WRAP" -eq 1 ]; then
  ...
  # The command line is committed as proof, so the home directory is
  # normalized to /home/user: the run must be identifiable, its operator must
  # not be.
  printf '%q ' "$@" | sed "s#${HOME}#/home/user#g" > "$RUN_DIR/command.txt"
  printf '\n' >> "$RUN_DIR/command.txt"
